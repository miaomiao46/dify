...
<Row>
  <Col>
    Retries every document in the specified dataset whose indexing status is `error` or `paused` and which is not archived. No document IDs need to be provided.
    ### Path
    <Properties>
      <Property name='dataset_id' type='string' key='dataset_id'>
        Knowledge Base ID
  ...
  <Col sticky>
    <CodeGroup
      title="Request"
      tag="POST"
      label="/datasets/{dataset_id}/retry"
      targetCode={`curl --location --request POST '${props.apiBaseUrl}/datasets/{dataset_id}/retry' \\\n--header 'Authorization: Bearer {api_key}'`}
    >
    ```bash {{ title: 'cURL' }}
    curl --location --request POST '${props.apiBaseUrl}/datasets/{dataset_id}/retry' \
    --header 'Authorization: Bearer {api_key}' \
    ```
    </CodeGroup>
    <CodeGroup title="Response">
...
<Heading
  ...
  method='GET'
  title='Download Document Source File'
...
<Row>
  <Col>
    ...
    <i>Directly returns the raw binary content of the document's original file (with <code>Content-Disposition: attachment</code>), without the extra round trip of first requesting a signed URL as in "Get Upload File". Only documents whose data source is <code>upload_file</code> are supported.</i>
  </Col>
  <Col sticky>
    <CodeGroup
      title="Request"
      tag="GET"
      label="/datasets/{dataset_id}/documents/{document_id}/download"
      targetCode={`curl --location --request GET '${props.apiBaseUrl}/datasets/{dataset_id}/documents/{document_id}/download' \\\n--header 'Authorization: Bearer {api_key}' \\\n--output downloaded_file`}
    >
    ```bash {{ title: 'cURL' }}
    curl --location --request GET '${props.apiBaseUrl}/datasets/{dataset_id}/documents/{document_id}/download' \
    --header 'Authorization: Bearer {api_key}' \
    --output downloaded_file
    ```
    </CodeGroup>
    <CodeGroup title="Response">
    ```text {{ title: 'Response' }}
    Binary file content (the raw document bytes)
    ```
    </CodeGroup>
  </Col>
</Row>
...
<Row>
  ...
  <Col sticky>
    <CodeGroup
      title="Request"
      tag="GET"
      label="/datasets/{dataset_id}/documents/{document_id}/metadata"
      targetCode={`curl --location --request GET '${props.apiBaseUrl}/datasets/{dataset_id}/documents/{document_id}/metadata' \\\n--header 'Authorization: Bearer {api_key}'`}
    >
    ```bash {{ title: 'cURL' }}
    ```
    </CodeGroup>
