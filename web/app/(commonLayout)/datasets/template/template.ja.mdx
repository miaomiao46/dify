...
<Heading
  ...
  method='POST'
  title='失敗したドキュメントを再試行'
  name='#retry_document'
/>
<Row>
  <Col>
    指定したナレッジベース内で、インデックスステータスが「エラー」または「一時停止」であり、アーカイブされていないすべてのドキュメントを再試行します。ドキュメント ID を指定する必要はありません。
    ### パス
    <Properties>
      <Property name='dataset_id' type='string' key='dataset_id'>
        ナレッジベース ID
...
<Row>
  <Col>
    ...
    <i>ドキュメントの元ファイルのバイナリデータをそのまま返します（<code>Content-Disposition: attachment</code>）。「アップロードファイルを取得」のように署名付き URL を別途取得する必要はありません。データソースが <code>upload_file</code> のドキュメントのみ対応しています。</i>
  </Col>
  <Col sticky>
    <CodeGroup
      title="リクエスト"
