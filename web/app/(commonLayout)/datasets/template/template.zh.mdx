...
<Row>
  <Col>
    重试指定知识库下所有索引状态为“失败”（error）或“暂停”（paused）且未归档的文档，无需传入文档 ID。
    ### Path
    <Properties>
      <Property name='dataset_id' type='string' key='dataset_id'>
        知识库 ID
...
<Row>
  <Col>
    ...
    <i>直接返回文档对应的原始文件二进制内容（响应头 <code>Content-Disposition: attachment</code>），无需像“获取上传文件”接口那样再请求一次签名 URL。仅支持数据来源为 <code>upload_file</code> 的文档。</i>
  </Col>
  <Col sticky>
    <CodeGroup
      title="Request"
